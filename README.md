...
- Prerendering HTML is also time consuming. To do so run `yarn start-prerender` in one terminal and then run `yarn html-fragments` in another. HTML fragments for the different routes are committed to source control in `client/src/prerendered-fragments`.
